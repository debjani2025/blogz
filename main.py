...
# Connect FLASK to MySQL server(connection string),

app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql+pymysql://blogz:productive@localhost:8889/blogz'

# Print SQL comms in terminal -objective relational mapping, connection flask and databases, debugging issues
app.config['SQLALCHEMY_ECHO'] = True

# Create python database object - new variable db set to SQLAlchemy constructor
db = SQLAlchemy(app)
...
def login():
    if request.method == 'POST':
        ...
        if user and user.password == password:
            ...
            flash('You aren logged in Succesfully ' + username , 'success')
            print(session)
...
def signup():
    if request.method == 'POST':
        ...
        if username == '':
            ...
            username_error = 'No space allowed in Username' 
        elif len(username) < 3 or len(username) >20:
            username_error = 'Username length should be between 3 and 20'
        elif username == existing_user:
            username_error = 'Name already exists'
        ...
        if password == '':
            ...
            password_error = 'Password length should be between 3 and 20'            
